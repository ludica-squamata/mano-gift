...
class ShadowSprite(AzoeSprite):
    ...
    def update_luces(self, event):
        p = event.data['light']
        if self.filtrar_luces(p):
            n = self.step + self.direccion
            self._luces[n][p] = 1
            self._luces[n][(p + 4) % 8] = 0  # las luces de lados contrarios se anulan

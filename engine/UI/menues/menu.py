from engine.globs import EngineData, ANCHO, ALTO, CAPA_OVERLAYS_MENUS, Colores
...
class Menu(EventAware, BaseWidget):
    ...
    def recolor(self, event):
        if event.data['name'] in ['CANVAS_BG', 'TEXT_SEL']:
            self.canvas.blit(self.create_raised_canvas(self.w, self.h), [0, 0])
            self.crear_titulo(self.titulo)